...
function getSkillDirectoryName(
  skill: Pick<SkillManifest, "concept" | "language">,
): string {
...
function applyNativeInstall(
  ...
  if (nativeConfig.strategy === "commands-dir") {
    ...
    const targetFile = path.join(
      commandsDir,
      `${skill.concept}-${skill.language}.md`,
    );
    ...
    fs.appendFileSync(
      targetFile,
      header + Object.values(fileContents).join("\n\n"),
    );
    return targetFile;
    ...
    const rootDir =
      nativeConfig.strategy === "dir" && nativeConfig.dir
        ? nativeConfig.dir
        : ".ai-skills";
...
export async function installSkill(
  ...
  try {
    ...
    if (agent && agentsConfig.agents[agent]) {
      installedAt = applyNativeInstall(
        skill,
        fileContents,
        agent,
        agentsConfig.agents[agent].nativeInstall,
      );
    } else if (agent) {
