...
function getLegacySkillId(
  id: Pick<{ concept: string; language: string }, "concept" | "language">,
): string {
...
export async function installAll() {
  ...
  const lockfile: SkillsLockfile = JSON.parse(
  ...
  for (const entry of lockfile.skills) {
    const skill = index.skills.find(
      (s) => s.id === entry.id || getLegacySkillId(s) === entry.id,
    );
    if (!skill) {
  ...
  if (successCount > 0)
    console.log(
      chalk.green(`  ${successCount} skill(s) installed successfully.`),
    );
...
export function initLockfile() {
  ...
  console.log(
    chalk.dim(
      'Add entries to the "skills" array, then run: ai-skills install-all',
    ),
  );
  console.log(
    chalk.dim(
      'Each entry needs an "id" (e.g. "testing-typescript"). The "agent" field is optional to install it native (e.g. "claude-code"). Legacy "concept/language" IDs are still accepted.',
    ),
  );
}
