...
export async function showSkillInfo(concept: string, lang: string) {
  ...
  if (!skill) {
    ...
    console.log(chalk.yellow('Run `ai-skills list` or `ai-skills search <query>` to find skills.'));
    process.exit(1);
  }
  ...
  console.log(`  ${chalk.bold('Language:')}    ${chalk.yellow(skill.language)}`);
  console.log(`  ${chalk.bold('Category:')}    ${chalk.magenta(skill.category)}`);
  console.log(`  ${chalk.bold('Tags:')}        ${skill.tags.map(t => chalk.gray(t)).join(', ')}`);
  ...
  if (agents.length > 0) {
    ...
    for (const [agentId, agentConfig] of agents) {
      ...
      let nativePath = '';
      if (cfg.strategy === 'commands-dir') nativePath = `${cfg.commandsDir}/${skill.concept}.md`;
